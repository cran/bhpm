...
Id <- "$Id: bhpm.cluster.BB.hier3.R,v 1.4 2019/05/28 11:05:07 clb13102 Exp clb13102 $"

bhpm.cluster.BB.hier3 <- function(cluster.data, sim_type = "SLICE", burnin = 20000, iter = 60000, nchains = 5,
	theta_algorithm = "MH",
...
{
	if (level == 0) {
		model_fit = bhpm.cluster.BB.indep(cluster.data, sim_type, burnin, iter, nchains,
						theta_algorithm, global.sim.params, sim.params, monitor,
						initial_values,
						hyper_params, global.pm.weight, pm.weights,
						adapt_phase, memory_model)
	} else if (level == 1) {
		model_fit = bhpm.cluster.BB.dep.lev1(cluster.data, sim_type, burnin, iter,
						nchains, theta_algorithm, global.sim.params, sim.params,
						monitor, initial_values,
						hyper_params, global.pm.weight, pm.weights,
						adapt_phase, memory_model)
	} else if (level == 2) {
		model_fit = bhpm.cluster.BB.dep.lev2(cluster.data, sim_type, burnin, iter,
						nchains, theta_algorithm, global.sim.params, sim.params,
						monitor, initial_values,
						hyper_params, global.pm.weight, pm.weights,
						adapt_phase, memory_model)
